...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    ppo_runner = WMPRunner(env, agent_cfg.to_dict(), log_dir=None, device=agent_cfg.device)
    ...
    trajectory_history = torch.zeros(size=(env.num_envs, history_length, env.num_obs - 3), device = env.device)
    ...
    while simulation_app.is_running():
        for i in range(1*int(env.max_episode_length) + 3):
            ...
            reset_env_ids = dones.nonzero(as_tuple=False).flatten().cpu().numpy()
