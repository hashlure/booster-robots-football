...
class ObservationsCfg:
    ...
    class AMPObsCfg(ObsGroup):
        # root_height = ObsTerm(func=mdp.base_pos_z, clip=(-100.0, 100.0), scale=1.0,)
        # root_wxyz = ObsTerm(func=mdp.root_quat_w, clip=(-100.0, 100.0), scale=1.0,)
        joint_pos = ObsTerm(func=mdp.joint_pos, clip=(-100.0, 100.0), scale=1.0,)
        joint_vel = ObsTerm(func=mdp.joint_vel, clip=(-100.0, 100.0), scale=1.0,)
...
class EventCfg:
    ...
    base_com = EventTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="Trunk"),
            "com_range": {"x": (-0.01, 0.01), "y": (-0.05, 0.05), "z": (-0.01, 0.01)},
        },
    )
...
class RewardsCfg:
    ...
    dof_vel = RewTerm(func=mdp.joint_vel_l2, weight=-0.002)
...
class TerminationsCfg:
    ...
    bad_orientation = DoneTerm(
        func=mdp.bad_stand_orientation,
        params={
            "limit_angle": 0.7,
