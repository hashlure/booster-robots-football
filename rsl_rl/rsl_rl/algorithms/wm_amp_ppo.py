...
from rsl_rl.storage import ReplayBuffer, WM_RolloutStorage
...
class WMAMPPPO:
    ...
    def __init__(
        ...
        self.amp_transition = WM_RolloutStorage.Transition()
        ...
        self.storage: WM_RolloutStorage = None  # type: ignore
        self.transition = WM_RolloutStorage.Transition()
    ...
    def init_storage(
        ...
        # create rollout storage
        self.storage = WM_RolloutStorage(
            training_type,
            num_envs,
    ...
    def act(self, obs, critic_obs, amp_obs, history, wm_feature):
        ...
        self.transition.actions = self.policy.act(aug_obs, history, wm_feature).detach()
        self.transition.values = self.policy.evaluate(aug_critic_obs, wm_feature).detach()
    ...
    def update(self):  # noqa: C901
        ...
        for sample, sample_amp_policy, sample_amp_expert in zip(generator, amp_policy_generator, amp_expert_generator):
            ...
            self.policy.act(aug_obs_batch, history_batch, wm_feature_batch, masks=masks_batch,
            ...
            value_batch = self.policy.evaluate(aug_critic_obs_batch, wm_feature_batch, masks=masks_batch,
            ...
            target_linear_vel = aug_critic_obs_batch[:, 0:3] # velocity
            vel_predict_loss = (predicted_linear_vel - target_linear_vel).pow(2).mean()
            loss += self.vel_predict_coef * vel_predict_loss
            # Symmetry loss
            if self.symmetry:
